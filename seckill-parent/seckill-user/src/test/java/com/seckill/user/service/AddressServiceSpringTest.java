...
/**
 * AddressService Spring Boot 集成测试
 */
@SpringBootTest
@ActiveProfiles("test")
@DisplayName("AddressService Spring Boot 集成测试")
class AddressServiceSpringTest {
    ...
    void setDefaultAddress_Success() {
        ...
        when(addressMapper.selectById(addressId)).thenReturn(address);
        when(addressMapper.cancelDefaultByUserId(userId)).thenReturn(1);
        when(addressMapper.setDefault(addressId, userId)).thenReturn(1);
